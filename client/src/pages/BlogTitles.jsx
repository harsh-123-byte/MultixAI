...
// frontend se backend call ke liye baseURL zaroori hai.
...
const BlogTitles = () => {
  ...
  const blogCategories = ['General',  'Technology', 'Business', 'Health', 'Lifestyle', 'Education', 'Travel', 'Food']
    ...
    const [selectedCategory, setSelectedCategory] = useState('General') // default selected category General wala rhega.
    const [input, setInput] = useState('')

    const [loading, setLoading] = useState(false)
    const [content, setContent] = useState('')

    const {getToken} = useAuth() // from clerk because clerk authentication hai.
    ...
    const onSubmitHandler = async (e)=>{
      ...
      try {
         ...
         if (data.success) {
          ...
          toast.error(data.message) // message from api response.
         }
      } catch (error) {
  ...
  return (
    // Iska bhi layout bilkul similar rhega as it is made in writeArticle page.
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
          ...
          <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
            {blogCategories.map((item)=>(
              ...
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedCategory === item ? 'bg-purple-50 text-purple-700' : 'text-gray-500 border-gray-300'}`} key={item}>{item}</span> // Jis category item pe click karenge wo wala purple ho jayega, baaki sab grey rhega.
            ) )}
          </div>
          <br/>
            ...
            </div>
            {
              !content ? (
                ...
                <div className='mt-3 h-full overflow-y-scroll text-sm text-slate-600'>
                  <div className='reset-tw'> {/* Markdown se pahele tailwind reset karna zaroori hai. */}
