...
const GenerateImages = () => {
  ...
  const imageStyle = ['Realistic', 'Ghibli style', 'Anime style', 'Cartoon style', 'Fantasy style', 'Realistic style', '3D style', 'Portrait style']
      ...
      // This codeblock before the return statement in same in every pages.
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
          ...
          <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
            ...
            {/* ek checkbox lagayenge so that if we want to make our image public or not matlab community section me show karana chahte ho ya nhi. */}
            <div className='m y-6 flex items-center gap-2'>
          ...
          <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#00AD25] to-[#04FF50] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
            {
            loading ? <span className='w-4 h-4 my-1 rounded-full border-2 border-t-transparent animate-spin'></span> // jab load ho rha hai to button disable kar ke spin animation laga do button pe.
            : <Image className='w-5'/>
            }
